...
class User(Base):
    ...
    indexes = relationship("ConsumptionIndex", back_populates="owner")
...
class Location(Base):
    ...
    indexes = relationship("ConsumptionIndex", back_populates="location")

class Category(Base):
    __tablename__ = "categories"
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String, unique=True, nullable=False) # e.g. "Electricity", "Water"
...
class Invoice(Base):
    ...
    consumption_value = Column(Float, nullable=True) # e.g. 150 kWh
